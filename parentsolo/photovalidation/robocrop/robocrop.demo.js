...
(function($) {
    ...
    RoboCrop.prototype.busy = function(e) {
        e === !0 ? $(this.imageElement).find(".loader").length || $(this.imageElement).prepend('<div class="loader"></div>').addClass("busy") : ($(this.imageElement).find(".loader").remove(), $(this.imageElement).removeClass("busy"))
        ...
        return this.container.on("mousedown touchstart", ".resize-handle", function(e) {
            ...
            $("#crop-editor .resize-container").show(), this.resizeImage(this.image.resolutions.auto.w, this.image.resolutions.auto.h), this.centerImage(), this.editorBusy(!1), this.cropInit(), $(o.imageElement).data("name", Math.random().toString(36).substring(7))
    ...
    }, RoboCrop.prototype.getProportionalDimensions = function(e, t) {
        var i = []
        e > this.image.max_width || t > this.image.max_height ? this.image.max_width < e ? (this.image.resolutions.max.w = this.image.max_width, this.image.resolutions.max.h = i.w / e * t, i.w = this.image.max_width, i.h = i.w / e * t) : (this.image.resolutions.max.h = this.image.max_height, this.image.resolutions.max.w = i.h / t * e, i.h = this.image.max_height, i.w = i.h / t * e) : (this.image.resolutions.max.w = e, this.image.resolutions.max.h = t, i.w = e, i.h = t)
        var o = .8
        return e > $("#crop-editor").width() || t > $("#crop-editor").height() ? (e > $("#crop-editor").width() && (this.image.resolutions.auto.w = Math.ceil($("#crop-editor").width() * o), this.image.resolutions.auto.h = Math.ceil($("#crop-editor").width() * o / e * t)), t > $("#crop-editor").height() && (this.image.resolutions.auto.w = Math.ceil($("#crop-editor").height() * o / t * e), this.image.resolutions.auto.h = Math.ceil($("#crop-editor").height() * o))) : (this.image.resolutions.auto.w = Math.ceil(e), this.image.resolutions.auto.h = Math.ceil(t)), this.image.resolutions.small.w = Math.ceil(this.image.resolutions.auto.w / 2), this.image.resolutions.small.h = Math.ceil(this.image.resolutions.auto.h / 2), i
    ...
    }, RoboCrop.prototype.resizeImage = function(e, t) {
        e = Math.ceil(e), t = Math.ceil(t), this.resize_canvas.width = e, this.resize_canvas.height = t, this.resize_canvas.getContext("2d").drawImage(this.orig_src, 0, 0, e, t), this.image_target.width = e, this.image_target.height = t, $("#crop-editor .resize-image").attr("width", Math.ceil(e)), $("#crop-editor .resize-image").attr("height", Math.ceil(t)), this.image_target.get(0).getContext("2d").drawImage(this.resize_canvas, 0, 0), $("#crop-editor .current-status").text(this.getImageSize().w + "x" + this.getImageSize().h), this.cropConflict() && ($("#crop-editor").find(".crop-area").remove(), this.cropValues.enabled = !1)
        ...
        if (i = void 0 === i ? 4 : i, this.cropBorder = i, this.cropValues.width.value = e, this.cropValues.height.value = t, o === !0) {
            var r = 0
            e += r, t += r, $("#crop-editor").find(".resize-container .crop-area").length || $("#crop-editor").find(".resize-container").prepend('<div class="crop-area"><span class="resize-crop top"></span><span class="resize-crop bottom"></span><span class="resize-crop left"></span><span class="resize-crop right"></span></div>'), $("#crop-editor").find(".crop-area").removeClass("resize-w"), $("#crop-editor").find(".crop-area").removeClass("resize-h"), this.cropValues.width.resize && $("#crop-editor").find(".crop-area").addClass("resize-w"), this.cropValues.height.resize && $("#crop-editor").find(".crop-area").addClass("resize-h"), $("#crop-editor .crop-area").css({
      ...
      this.editorBusy(!0), setTimeout(function() {
            var e = this.image_target.width,
                t = this.image_target.height
            this.resizeImage(this.image.resolutions.max.w, this.image.resolutions.max.h)
			//alert(this.image.resolutions.max.w);
			//alert(this.image.resolutions.max.h);
			
            var i = this.image_target.get(0).getContext("2d"),
			    o = i.canvas.width,
                r = i.canvas.height
				
            i.clearRect(0, 0, o, r), i.save(), i.translate(o/2, r/2); i.rotate(Math.PI/2), i.drawImage(this.orig_src, -o/2, -r/2), i.restore(), this.orig_src.src = this.image_target.get(0).toDataURL("image/png"), this.resizeImage(e, t),this.editorBusy(!1)
			//alert(this.orig_src);
			//alert(-1 * r);
			//alert(e);
			//alert(t);
		...
		angle = (angle+90)%360;
		alert(angle);
		canvas.ClassName = "rotate"+angle;
		alert("going");
		$('head').append('<link rel="stylesheet" href="photovalidation/robocrop/style2.css" type="text/css" />'); */
    ...
    $(".crop-element").each(function() {
        var e = $(this).closest(".crop-element").find("img").attr("src")
        void 0 !== e && e.length <= 0 && $(this).addClass("not-empty")
    }), $(document).on("click", ".btn-crop-tool.flip-x,.btn-main-menu-action.flip-x", function() {
        robocrop.flipX()
    }), $(document).on("click", ".btn-crop-tool.flip-y,.btn-main-menu-action.flip-y", function() {
        robocrop.flipY()
    }), $(document).on("click", ".btn-crop-tool.flip-z,.btn-main-menu-action.flip-z", function() {
        robocrop.flipZ()
    }),	$(document).on("click", ".btn-crop-tool.invert,.btn-main-menu-action.invert", function() {
        robocrop.invertDimensions()
    }), $(document).on("click", ".btn-crop-tool.crop,.btn-main-menu-action.crop", function() {
        robocrop.toggleCrop()
    }), $(document).on("click", ".crop-element .edit", function(e) {
        e.preventDefault(), e.stopPropagation()
        var t = $(this).closest(".crop-element").find("img").attr("src")
        void 0 !== t && t.length > 0 && (robocrop.clearImage(), robocrop.imageElement = $(this).closest(".crop-element"), robocrop.loadImageFromUrl(t), $("#crop-editor").addClass("open"))
    }), $("#crop-editor").on("dragover", function(e) {
        e.stopPropagation(), e.preventDefault()
    }), $("#crop-editor").on("drop", function(e) {
        e.stopPropagation(), e.preventDefault(), robocrop.clearImage(), robocrop.dropImage(e)
    }), $(document).on("click", "#crop-editor.busy", function(e) {
        e.preventDefault(), e.stopPropagation()
    }), $(document).on("click", "#crop-editor .image-resolutions .img-max", function() {
        robocrop.resizeImage(robocrop.image.resolutions.max.w, robocrop.image.resolutions.max.h), robocrop.centerImage()
    }), $(document).on("click", "#crop-editor .image-resolutions .img-auto", function() {
        robocrop.resizeImage(robocrop.image.resolutions.auto.w, robocrop.image.resolutions.auto.h), robocrop.centerImage()
    }), $(document).on("click", "#crop-editor .image-resolutions .img-small", function() {
        robocrop.resizeImage(robocrop.image.resolutions.small.w, robocrop.image.resolutions.small.h), robocrop.centerImage()
    }), $(document).on("click", ".crop-element.busy", function(e) {
        e.preventDefault(), e.stopPropagation()
    }), $(document).on("click", "#crop-editor .editor-main-menu .cancel", function() {
        robocrop.clearImage(), $("html,body,#crop-editor").css("overflow", "initial"), $("#crop-editor").removeClass("open")
    }), $(document).on("change", ".crop-element input:file", function() {
        robocrop.clearImage(), robocrop.loadImage(this), $("#crop-editor").addClass("open")
    }), $(document).on("click", "#crop-editor .editor-main-menu .apply", function() {
        robocrop.cropValues.enabled && (robocrop.crop(), robocrop.toggleCrop(!1))
        var e = $(robocrop.imageElement).data("crop-required"),
            t = $("#crop-editor .resize-image").width(),
            i = $("#crop-editor .resize-image").height(),
            o = robocrop.validCropSize(t, robocrop.cropValues.width),
            r = robocrop.validCropSize(i, robocrop.cropValues.height)
        if (e === !0 && (t != o || i != r)) return void $("#crop-editor .current-status").text("RoboCrop is required")
        var a = $(robocrop.imageElement).data("watermark")
        void 0 !== name && void 0 !== robocrop.watermarks[a] ? robocrop.addWatermark(robocrop.watermarks[a]) : robocrop.apply(), $("#crop-editor").removeClass("open")
    }), $("#crop-editor").bind("mousewheel", function(e) {
        if (!$("#crop-editor").hasClass("busy")) {
            var t = parseInt(robocrop.getImageSize().w),
                i = parseInt(robocrop.getImageSize().h)
            e.originalEvent.wheelDelta / 120 > 0 ? robocrop.resizeImage(t + robocrop.image.zoom.speed, i + robocrop.image.zoom.speed) : t - robocrop.image.zoom.speed > 50 && i - robocrop.image.zoom.speed > 50 && robocrop.resizeImage(t - robocrop.image.zoom.speed, i - robocrop.image.zoom.speed), robocrop.centerImage()
        }
    }), $("#crop-editor").bind("DOMMouseScroll", function(e) {
        if (!$("#crop-editor").hasClass("busy")) {
            var t = parseInt(robocrop.getImageSize().w),
                i = parseInt(robocrop.getImageSize().h)
            return e.originalEvent.detail > 0 ? robocrop.resizeImage(t + robocrop.image.zoom.speed, i + robocrop.image.zoom.speed) : t - robocrop.image.zoom.speed > 50 && i - robocrop.image.zoom.speed > 50 && robocrop.resizeImage(t - robocrop.image.zoom.speed, i - robocrop.image.zoom.speed), robocrop.centerImage(), !1
        }
    })

    //var img_hw = document.getElementsByClassName('current-status');
    //alert('img'+img_hw);
