<?php
	...
	if($action == 'send') {
		...
		// recherche des destinataires
		$query = "SELECT u.id, u.username, u.email, up.genre, up.nom_origine, up.langue_appel"
		." FROM user AS u"
		." INNER JOIN user_notification AS un ON un.user_id = u.id"
		." INNER JOIN user_profil AS up ON up.user_id = u.id"
		.$_where
		...
		foreach($users as $user) {
			...
			if(in_array($user->langue_appel,[1,0])){
				...
				elseif($user->genre == 'f')
					$mailingTexte = str_replace('{cher_titre}', 'Ch&egrave;re Madame', $mailingTexte);
			}elseif($user->langue_appel == 2){
				if($user->genre == 'h')
					$mailingTexte = str_replace('{cher_titre}', 'Dear Sir', $mailingTexte);
				elseif($user->genre == 'f')
					$mailingTexte = str_replace('{cher_titre}', 'Dear Madam', $mailingTexte);
			}elseif($user->langue_appel == 3){
				if($user->genre == 'h')
					$mailingTexte = str_replace('{cher_titre}', 'Sehr geehrter Herr', $mailingTexte);
				elseif($user->genre == 'f')
					$mailingTexte = str_replace('{cher_titre}', 'Sehr geehrte Frau', $mailingTexte);
			}else
				$mailingTexte = str_replace('{cher_titre}', '', $mailingTexte);
			
			$mailingTexte = str_replace('{nom}', $user->nom_origine, $mailingTexte);
