<?php
	...
	class HTML_mailing {	
		...
		public static function mailingLister(&$rows, &$search, &$messages) {
			...
			function cancelsubmit(action) {
				...
				<div class="row">
                  <div class="col-lg-12">				
					<div class="toolbar">
						<a href="<?php echo SITE_URL_ADMIN; ?>/index.php?app=mailing&action=edit" title="R&eacute;diger un nouveau mailing" class=" btn btn-success">Nouveau</a>
				...
				<div class="tableAdmin">
					<table cellpadding="0" cellspacing="0" class="table table-bordered table-striped table-condensed cf lister">
						...
						<?php 						foreach($rows as $row) {
							...
							<tr class="list">
								...
								<td>
									<a href="<?php echo SITE_URL_ADMIN; ?>/index.php?app=mailing&action=edit&id=<?php echo $row->id; ?>" title="Modifier le mailing"><?php echo $row->nom; ?></a>
								</td>
								<td><?php echo date('d/m/Y � H:i:s', strtotime((string) $row->datetime_update)); ?></td>
								<td align="center">
									<a href="<?php echo JL::url('index.php?app=mailing&action=envoyer&id='.$row->id); ?>" title="Envoyer ce mailing"><img src="<?php echo SITE_URL_ADMIN; ?>/images/mailing.png" alt="" />
								</td>
							</tr>
							<?php 						}
						?>
						...
						<tr>
							<td colspan="<?php echo $tdParTr; ?>">
								<b>Pages</b>:
								<?php if($debut > 1) { ?> <a href="<?php echo JL::url(SITE_URL_ADMIN.'/index.php?app=mailing&search_at_page=1'); ?>" title="Afficher la page 1">D&eacute;but</a> ...<?php }?>
								<?php 									for($i=$debut; $i<=$fin; $i++) {
									?>
										 <a href="<?php echo JL::url(SITE_URL_ADMIN.'/index.php?app=mailing&search_at_page='.$i); ?>" title="Afficher la page <?php echo $i; ?>" <?php if($i == $search['page']) { ?>class="displayActive"<?php } ?>><?php echo $i; ?></a>
									<?php 									}
								?>
								<?php if($fin < $search['page_total']) { ?> ... <a href="<?php echo JL::url(SITE_URL_ADMIN.'/index.php?app=mailing&search_at_page='.$search['page_total']); ?>" title="Afficher la page <?php echo $search['page_total']; ?>">Fin</a><?php }?> <i>(<?php echo $search['result_total']; ?> r&eacute;sultats)</i>
							</td>
						</tr>
					<?php } else { ?>
						<tr>
					...
					<input type="hidden" name="app" value="mailing" />
		...
		public static function mailingEdit(&$row, &$list, &$messages) {
			...
			function cancelsubmit(action) {
						...
						if(ok) {
							...
							document.location = "<?php echo SITE_URL_ADMIN; ?>/index.php?app=mailing"; 
						}
			...
			<form name="editForm" action="<?php echo SITE_URL_ADMIN; ?>/index.php" method="post">
				<section class="panel">
                  ...
                  <div class="col-lg-12">				
					<div class="toolbar">
					<?php if($row->id){?><a href="<?php echo SITE_URL_ADMIN; ?>/index.php?app=mailing&action=apercu&id=<?php echo $row->id; ?>" target="_blank" title="Apercu" class=" btn btn-success">Aper&ccedil;u</a><?php } ?>
					...
					<table cellpadding="0" cellspacing="0" class="table table-bordered table-striped table-condensed cf editer">
						...
						<tr>
							...
							<td style="font-size:11px;">
								...
								<br />
								<table cellpadding="0px" cellspacing="5px" class="table table-bordered table-striped table-condensed cf">
		...
		public static function mailingSend(&$row, &$list) {
			...
			<script type="text/javascript" src="<?php echo SITE_URL; ?>/js/app_mailing.js"></script>
				...
				<div class="row">
                  <div class="col-lg-12">				
					<div class="toolbar">
						...
						<a href="<?php echo SITE_URL_ADMIN; ?>/index.php?app=mailing" title="Annuler" class=" btn btn-success">Annuler</a>
						<a href="<?php echo SITE_URL_ADMIN; ?>/index.php?app=mailing" title="Fermer" class="btn btn-danger">Fermer</a>
					</div>				
				  </div>
              </div>
				<br />
