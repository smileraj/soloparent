<?php
	...
	class HTML_mailing {	
		...
		public static function mailingLister(&$rows, &$search, &$messages) {
			...
			function cancelsubmit(action) {
				...
				<div class="row">
                  <div class="col-lg-12">				
					<div class="toolbar">
						<a href="<? echo SITE_URL_ADMIN; ?>/index.php?app=mailing&action=edit" title="R&eacute;diger un nouveau mailing" class=" btn btn-success">Nouveau</a>
					...
					<table cellpadding="0" cellspacing="0" class="table table-bordered table-striped table-condensed cf lister">
						...
						foreach($rows as $row) {
							...
							<tr class="list">
								...
								<td>
									<a href="<? echo SITE_URL_ADMIN; ?>/index.php?app=mailing&action=edit&id=<? echo $row->id; ?>" title="Modifier le mailing"><? echo $row->nom; ?></a>
								</td>
								<td><? echo date('d/m/Y à H:i:s', strtotime($row->datetime_update)); ?></td>
								<td align="center">
									<a href="<? echo JL::url('index.php?app=mailing&action=envoyer&id='.$row->id); ?>" title="Envoyer ce mailing"><img src="<? echo SITE_URL_ADMIN; ?>/images/mailing.png" alt="" />
								</td>
							</tr>
							<?
						}
						...
						<tr>
							<td colspan="<? echo $tdParTr; ?>">
								<b>Pages</b>:
								<? if($debut > 1) { ?> <a href="<? echo JL::url(SITE_URL_ADMIN.'/index.php?app=mailing&search_at_page=1'); ?>" title="Afficher la page 1">D&eacute;but</a> ...<? }?>
								<?
									for($i=$debut; $i<=$fin; $i++) {
									?>
										 <a href="<? echo JL::url(SITE_URL_ADMIN.'/index.php?app=mailing&search_at_page='.$i); ?>" title="Afficher la page <? echo $i; ?>" <? if($i == $search['page']) { ?>class="displayActive"<? } ?>><? echo $i; ?></a>
									<?
									}
								?>
								<? if($fin < $search['page_total']) { ?> ... <a href="<? echo JL::url(SITE_URL_ADMIN.'/index.php?app=mailing&search_at_page='.$search['page_total']); ?>" title="Afficher la page <? echo $search['page_total']; ?>">Fin</a><? }?> <i>(<? echo $search['result_total']; ?> r&eacute;sultats)</i>
							</td>
						</tr>
					<? } else { ?>
						<tr>
		...
		public static function mailingEdit(&$row, &$list, &$messages) {
			...
			function cancelsubmit(action) {
						...
						if(ok) {
							...
							document.location = "<? echo SITE_URL_ADMIN; ?>/index.php?app=mailing"; 
						}
			...
			<form name="editForm" action="<? echo SITE_URL_ADMIN; ?>/index.php" method="post">
				<section class="panel">
                  ...
                  <div class="col-lg-12">				
					<div class="toolbar">
					<? if($row->id){?><a href="<? echo SITE_URL_ADMIN; ?>/index.php?app=mailing&action=apercu&id=<? echo $row->id; ?>" target="_blank" title="Apercu" class=" btn btn-success">Aper&ccedil;u</a><? } ?>
		...
		public static function mailingSend(&$row, &$list) {
			...
			<script type="text/javascript" src="<? echo SITE_URL; ?>/js/app_mailing.js"></script>
				...
				<div class="row">
                  <div class="col-lg-12">				
					<div class="toolbar">
						...
						<a href="<? echo SITE_URL_ADMIN; ?>/index.php?app=mailing" title="Annuler" class=" btn btn-success">Annuler</a>
						<a href="<? echo SITE_URL_ADMIN; ?>/index.php?app=mailing" title="Fermer" class="btn btn-danger">Fermer</a>
					</div>				
				  </div>
              </div>
				<br />
