<?php
	...
	class homeModel extends JLModel {
		...
		function display(){
			...
			//***Photos de profil par ordre décroissant
			...
			/*
			$num = JL::getSessionInt('colonne_4',0) % 2;
			...
			//colonne 4
			$query = "Select * from home_".$_GET['lang']." WHERE id =  '".$db->escape($id_colonne4)."'";
			$this->colonne_4 = $db->loadObject($query);
			
			JL::setSession('colonne_4',($num+1));
			*/
			//*Partie gauche
			
			
			//**Récup les dernières actu
			$query = "SELECT id, titre_fr as titre, date_add"
			." FROM contenu"
			...
			//**Récupère les données sur Babybook
