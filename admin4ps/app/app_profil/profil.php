	...
	function profilSave() {
		...
		if(!count($messages)) {
			...
			if($data->id) {
				...
				$updatedate=$data->gold_limit_date;
				//current date
				$yearIni = date("Y", strtotime($currentdate));
                $monthIni = date("m", strtotime($currentdate));
				//saved date
				$yearFin = date("Y", strtotime((string) $updatedate));
                 $monthFin = date("m", strtotime((string) $updatedate));
if ($yearIni == $yearFin) {
   $numberOfMonths = ($monthFin-$monthIni);
} else {
   $numberOfMonths = ((($yearFin - $yearIni) * 12) - $monthIni)+ $monthFin;
}
 ...
 $points_id=19;
 $points=30;
 JL::addPoints($points_id, $data->id);
 ...
 $messages[]	= '<span class="valid">'.$points.' points cr&eacute;dit&eacute;(s).</span>';
  ...
  $messages[]	= '<span class="valid">'.$points.' points cr&eacute;dit&eacute;(s).</span>';
				...
				if($data->confirmed == 0) {
					$updatePublished = ', published = 0, user_status_code = 0 ';
				} else {
					$updatePublished = ', published = 1, user_status_code = 1 ';
				}
	...
	function profilSuppression(){
		...
		if (is_array($ids)) {
			...
			$query = "UPDATE user_profil SET user_active=0 where user_id IN (".$in_id.")";
			$db->query($query);
	...
	function profilLister() {
		...
		$query = "SELECT u.id, u.username, u.confirmed, u.published, u.user_status_code, u.ip, u.ip_pays, up.helvetica, up.genre, us.gold_limit_date, up.nom_origine, up.prenom_origine, up.telephone_origine, us.appel_date, us.appel_date2, us.points_total"
		." FROM user AS u"
		." INNER JOIN user_profil AS up ON up.user_id = u.id"
		." INNER JOIN user_stats AS us ON us.user_id = u.id"
		.$_where
