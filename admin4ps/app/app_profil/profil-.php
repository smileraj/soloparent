<?php
	...
	function profilSave() {
		...
		if(!count($messages)) {
			...
			if($data->id) {
				...
				." prenom = '".$db->escape($data->prenom)."',"
				." telephone = '".$db->escape($data->telephone)."',"
				." adresse = '".$db->escape($data->adresse)."',"
				." code_postal = '".$db->escape($data->code_postal)."',"
	...
	function profilEditer() {
		...
		if(!$data->id) {
			...
			$query = "SELECT datetime"
			." FROM paypal"
			." WHERE user_id = '".$userObj->id."' AND valide = 1"
			." ORDER BY datetime DESC"
			." LIMIT 0,1"
			;
			$abonnement_carte = $db->loadResult($query);
			$userObj->abonnement_carte = $abonnement_carte ? $abonnement_carte : false;
			
			
			// récup les 4 actions d'abonnement, id en dur !
			$query = "SELECT id, nom, points"
			." FROM points"
	...
	function profilLister() {
		...
		$search['order']		= JL::getVar('search_order', JL::getSession('search_order', 'u.username'));
		$search['ascdesc']		= JL::getVar('search_ascdesc', JL::getSession('search_ascdesc', 'asc'));
		...
		if($search['word']) {
			$where[]			= "(u.username LIKE '%".$search['word']."%' OR u.email LIKE '%".$search['word']."%' OR up.nom LIKE '%".$search['word']."%' OR up.prenom LIKE '%".$search['word']."%' OR up.telephone LIKE '%".$search['word']."%')";
		}
		...
		$query = "SELECT u.id, u.username, u.confirmed, u.published, u.ip, u.ip_pays, up.helvetica, up.genre, us.gold_limit_date, up.nom, up.prenom, up.telephone, us.appel_date, us.appel_date2, us.points_total"
		." FROM user AS u"
		." INNER JOIN user_profil AS up ON up.user_id = u.id"
		." INNER JOIN user_stats AS us ON us.user_id = u.id"
		.$_where
