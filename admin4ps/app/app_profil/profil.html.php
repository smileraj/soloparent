				...
				<div class="tableAdmin">
					...
					<table cellpadding="0" cellspacing="0" class="table table-bordered table-striped table-condensed cf lister">
						...
						<tr>
							...
							<th align="center">Email Activ&eacute;</th>
							<th>Abonnement</th>
							<th>Nom</th>
							<th>Pr&eacute;nom</th>
						...
						?>
							<tr class="list<? if($jours > 0 && $jours <= 3) { ?> jours3<? } ?><? if($warning) { ?> warning<? } ?>" >
								...
								*/ ?>
								...
								<td align="center"><img src="images/<? echo $user->user_status_code; ?>.png" alt="<? echo $user->user_status_code ? 'Oui' : 'Non'; ?>" /></td>
							...
							<tr class="list_photos<? if($jours > 0 && $jours <= 3) { ?> jours3<? } ?><? if($warning) { ?> warning<? } ?>" >
								<td colspan="12">
									<?
											...
											?>
												<div class="col-md-1 col-sm-2 col-xs-3"><div class="profilPhoto">
													<img src="<? echo SITE_URL; ?>/images/profil/<? echo $user->id; ?>/<? echo $photo; ?>" alt="" width='100%'/>
												</div></div>
											<?
											}
										?>
						...
						foreach($users as $user) {
							if(count($user->photos)) {
								foreach($user->photos as $photo) {
									...
									<td >
									<label for="<? echo $user->user_id; ?>_<? echo $photo; ?>">											
											<div class="figure-wrapper"  >
											<input type="hidden" name="upload_dir_<?php echo $imgval;?>" id="upload_dir_<?php echo $imgval;?>" value="<? echo $user->user_id; ?>" />
											<input type="hidden" name="photo_id_<?php echo $imgval;?>" id="photo_id_<?php echo $imgval;?>" value="<? echo $photo; ?>" />
											<figure class=" image-container target ajaximg_<?php echo $imgval; ?>_img"  > 
											<img class="ajaximgcls_<?php echo $imgval;?>" src="<? echo SITE_URL; ?>/images/profil/<? echo $user->user_id; ?>/pending-parent-solo-<? echo $photo; ?>.jpg?<? echo time(); ?>"   id="target_<?php echo $imgval; ?>"/>
											</figure>
											</div>
											<br />
