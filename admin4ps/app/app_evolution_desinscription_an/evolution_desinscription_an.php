<?php
	...
	require_once('evolution_desinscription_an.html.php');
	...
	function lister() {
		...
		$query = "SELECT"
		." count(u.id) AS total,"
		." DATE_FORMAT(ud.datetime,'%Y') AS annee,"
		...
		." GROUP BY DATE_FORMAT(ud.datetime,'%Y')"
		." ORDER BY DATE_FORMAT(ud.datetime,'%Y') ASC"
		;
		...
		evolution_desinscription_an_HTML::lister($evolution, $evolution_suppr);
