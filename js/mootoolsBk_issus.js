...
function windowOpen(title, url, width, height, scroll){if(scroll == null){scroll='no';} var win = window.open(url, title, 'width='+width+',height='+height+',resizable=no,scrollbars='+scroll+',menubar=no,status=no');win.focus();}
var sonJoue	= 0;
function chatGetNewConversations(site_url, user_id_from, key) {
	var timerAlert;
	if(timerAlert == null) {
		timerAlert = 0;
	}
	...
	new Request(
	{
		url: site_url+'/ajaxChat.php',
		method: 'get',
		headers: {'If-Modified-Since': 'Sat, 1 Jan 2000 00:00:00 GMT'},
		data: {
				"user_id_from": user_id_from,
				"key": key,
				"action": 'getNewMessagesNb'
		},
		onSuccess: function(ajax_return) {
			var messagesNb = '';
			messagesNb = ajax_return;

			if(sonJoue == 1) {
				swfobject.removeSWF('chatSon');
			}
			...
			if(messagesNb != '' && messagesNb != '0') {
				...
				if(sonJoue == 0) {
					swfobject.embedSWF(site_url+"/images/chatSon.swf", "chatSon", "1", "1", "8.0.0", "", { "width": "1", "height": "1" }, { "autoplay": "true"}, { "id" : "chatSon" });
					sonJoue = 1;
				}

				try{if(timerAlert == 0){chatAlert(messagesNb);}}catch(e){}
			} else {
...
function noPhotoPopIn(i) {
	...
	if(i < 3) {
		timerAlert2 = setInterval('noPhotoPopIn('+(i+1)+')', 1500);
	}
}


function positionPopIn(element, xRight, yBottom, move) {

	try {

		// decalage element
		var elementLeft		= -xRight;
		var elementTop		= -yBottom;
		...
		if(!innerWidth) { // IE
			...
			elementLeft		= elementLeft + scrollXY[0];
			elementTop		= elementTop + scrollXY[1];

		} else { // bons navigateurs

			$(element).style.position = 'fixed';
			if(innerWidth > clientWidth) {
				elementLeft = elementTop - 15;
			}
		...
		// nouvelles coordonnees de l'element
		var posX = innerWidth + elementLeft - elementWidth;
		var posY = innerHeight + elementTop - elementHeight;
