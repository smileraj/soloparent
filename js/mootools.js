...
function windowOpen(title, url, width, height, scroll) {
    if (scroll == null) {
        scroll = 'no';
    }
    var win = window.open(url, title, 'width=' + width + ',height=' + height + ',resizable=no,scrollbars=' + scroll + ',menubar=no,status=no');
    win.focus();
}
var sonJoue = 0;
    ...
    new Request({
        ...
        onSuccess: function(ajax_return) {
            ...
            if (messagesNb != '' && messagesNb != '0') {
                ...
                if (sonJoue == 0) {
                    swfobject.embedSWF(site_url + "/images/chatSon.swf", "chatSon", "1", "1", "8.0.0", "", {
                        "width": "1",
                        "height": "1"
                    }, {
                        "autoplay": "true"
