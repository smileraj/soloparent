function rechercheDestinataires() {
	try {
		...
		if($('envoiEnCours').value == 0) {
			new Request(
			{
				url: $('site_url').value+'/app/app_mailing2/ajax.php',
				method: 'get',
				...
				onFailure: function(){
					$('loading').style.visibility = "hidden";
					if(confirm('Une erreur s\'est produite lors de la recherche de destinataires... Voulez-vous essayer � nouveau ?')) {
...
function mailingEnvoi(serie) {
	if(serie == null) {
		...
		if($('#destinatairesNb').value == 0) {
			alert('Votre recherche n\'a retourn� aucun destinataire ! Envoi impossible.');
			return 0;
		}
		if(confirm('Attention, vous �tes sur le point d\'envoyer $('#d'destinatairesNb').value+' email(s) ! Confirmez-vous ?')) {
	...
	try {
		...
		new Request(
		{
			...
			onFailure: function() {
				$('#destinataires').innerHTML = 'Un erreur est survenue lors de l\'envoi de la s�rie de mails num�ro '+serie+' !, le mailing a �t� interrompu.';
				$('loading2').style.visibility = "hidden";
			}
		}
		).send();
