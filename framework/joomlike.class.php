...
ini_set('MAX_EXECUTION_TIME', -1);
    ...
    return $obj;
		...
		// vérifie si l'application à charger existe, et que le visiteur n'essaye pas divers hacks dans l'url avec la variable $app
		public static function checkAppExpert($app_load) {
			// variables
			$app_ok	= true;
			// on conserve uniquement les caractères alphanumériques et _
			$app_load_check	= preg_replace('#[^a-z0-9_]#', '', $app_load);
		...
		public static function loadBody($admin = '') {
			global $app;
			JL::loadApp($app, $admin);
		}
		
		// charge le corps de la page, en précisant en param si c'est une appli $admin (charge par défaut le $app)
		public static function loadBodyExpert() {
			global $app;
		...
		public static function loadApp($app_load, $admin = '') {
			// détermine si c'est une appli admin ou visiteur
			if(!$admin) { // app coté front
				$path	= SITE_PATH;
			} else { // app coté back
		...
		// charge l'application passée en param
		public static function loadAppExpert($app_load) {
			// détermine si c'est une appli admin ou visiteur
			$path	= SITE_PATH_ADMIN_EXPERT;
		...
		public static function loadMod($mod, $admin = '') {
			//die($langue);
			global $langue;
			// détermine si c'est une appli admin ou visiteur
			if(!$admin) { // app coté front
    ...
    return makeSafe($value ?? '', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
		...
		public static function getSessionInt($key, $defaut) {
		...
		public static function mail($destinataire, $titre, $texte, $utf8 = true) {
			// headers		
		include("./PHPMailer/PHPMailerAutoload.php"); //Here magic Begen we include PHPMailer Library.
        include("./PHPMailer/class.phpmailer.php");  
		//$destinataire1='jravichandar@esales.in';
		$mail = new PHPMailer;
		$mail -> CharSet = 'UTF-8';
...
echo "<script type='text/javascript'>console.log('$message');</script>";
		...
		public static function getUploadDir($base, $upload_dir = '') {
		...
		public static function userGetPhoto($user_id, $photo_format, $photo_type, $photo_i) {
			// variables
		...
		public static function addLastEvent($user_id = 0, $last_event_user_id = 0, $last_event_type = 0, $last_event_data = 0) {
			global $db;
		...
		public static function addPoints($points_id, $user_id, $data = '') {
			global $db;
		...
		public static function rankingPoints() {
			global $db;
		...
		public static function winnerPoints($annee_mois, $position, &$row) {
			global $langue;
			include("lang/app_framework.".$_GET['lang'].".php");
			global $db;
...
if($winner) {
    ...
    $db->query($query);
		...
		public static function getMailHtml($templatePath, $titre = '', $texte = '', $username = '', $var = null) {
		...
		*/
		public static function makeSelectList( &$arr, $tag_name, $tag_attribs, $key, $text, $selected=NULL ) {
			// check if array
			if ( is_array( $arr ) ) {
				reset( $arr );
			}
