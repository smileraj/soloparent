...
ini_set('MAX_EXECUTION_TIME', -1);
	class JL {
		...
		// g�n�re la clause where � partir d'un tableau $where pass� en param
		...
		// cr�e le dossier d'upload temporaire et retourne son chemin
		...
		function notificationBasique($type = 'visite', $profil_id) {
			...
			if((int)$profil->{'new_'.$type}) {
				...
				// int�gration du texte et du template, ainsi que traitement des mots cl�s
		...
		*/
		...
		function calcul_age(&$date_naissance){
			...
			$age_annee = $today_annee - $annee_naiss;//diff�rence entre les ann�es
			
			//si nombre ann�e sup�rieur ou �gal � 2 ans
			if($age_annee >= 3){
				
				//on v�rifie si l'anniversaire du participant n'est pas encore pass�
				//on v�rifie si le mois du jour est avant ou le m�me (si avant on retire un an)
				if($today_mois <= $mois_naiss){
					
					//si c'est le m�me mois on v�rifie si le jour est avant (si oui, on retire un an)
					if($today_mois == $mois_naiss){
				...
				$age_mois = $today_mois - $mois_naiss;//diff�rence entre les mois
				
				//on ajoute les ann�es en mois si + d'un an
				if($age_annee >= 2)
					$age_mois += 24;
				elseif($age_annee >= 1)
					$age_mois += 12;
				
				//on v�rifie si le jour est avant (si oui, on retire un mois)
				if($today_jour < $jour_naiss)
					$age_mois--;
