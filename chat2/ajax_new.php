<?php
	...
	$texte = mb_convert_encoding(JL::getVar('texte', ''), 'UTF-8', 'ISO-8859-1');
	switch($action) {
	...
	function getUtilisateur(){
		...
		if (intval(($utilisateur->last_online_time) > (ONLINE_TIME_LIMIT + AFK_TIME_LIMIT)) && ($utilisateur->on_off_status==0)) {
	...
	?>
		<div class='chatProfileFromImg'>
			...
			<div><?php echo $utilisateur->on_off_status; ?></div>
		</div>
	...
	function getCorrespondant($id_corresp){
		...
		if(!$blacklists){
			...
			//a supprimer une fois le module en ligne refait
			//updated online status if they put offline
			if (intval(($correspondant->last_online_time) > (ONLINE_TIME_LIMIT + AFK_TIME_LIMIT))&& ($correspondant->on_off_status==0)) {
		...
		?>
		 ...
		 <div class="top chatboxhead">
			<div class="col-md-6 col-sm-6 col-xs-6  parentsolo_plr_0"> <div class="col-md-3  col-sm-3 col-xs-3  parentsolo_plr_0 parentsolo_mt_20">  <span style="">
					...
					<img src='<?php echo $correspondant->photoURL; ?>' alt='profil' /></span>
...
> <span  class="name nickname<?php echo $correspondant->genre; ?>"><?php echo $correspondant->username; ?></span><br>
					<span class='online_<?php echo $correspondant->online; ?>'><?php echo $online_2; ?></span>
                </span></div></div>
		...
		<?php 							
			...
			if($blacklists){
				...
				foreach($messages as $message){
					...
					$message->texte = mb_convert_encoding((string) $message->texte, 'UTF-8', 'ISO-8859-1');
				?>
...
print_r($matches);
		...
		$texte = mb_convert_encoding((string) $texte, 'ISO-8859-1');
