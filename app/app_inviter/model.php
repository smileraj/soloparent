<?php
	...
	class fairepartModel extends JLModel {
		...
		// r&eacute;cup les donn&eacute;es
		function getData() {
			global $langue;
			global $db, $user;
			...
			// donn&eacute;es modifiables par l'utilisateur
			...
			//formulaire pour envoy&eacute; aux amis
		...
		// v&eacute;rifie les donn&eacute;es
		function checkData() {
			global $langue;
		...
		function edit() {
			...
			if($this->_data->id) {
				...
				// si donn&eacute;es trouv&eacute;es
				if($_data) {
					$this->_data 		= $_data;
				} else { // sinon reset l'id des valeurs envoy&eacute;es
					$this->_data->id 	= 0;
				}
		...
		// active/d&eacute;sactive un enregistrement
		function active($id) {
			global $langue;
			global $db, $user;
		...
		function listTemplate($template = 0, $js = '') {
			...
			if(is_dir($dir)) {
				...
				while($file = trim(readdir($dir_id))) {
					...
					// r&eacute;cup les miniatures de photos valides
			...
			// r&eacute;ordonne le tableau par ordre alphab&eacute;tique
			sort($listTemplate);

			// g&eacute;n&egrave;re la liste d&eacute;roulante
			$this->_list['template'] = JL::makeSelectList($listTemplate, 'template', 'id="template" '.$js, 'value', 'text', $template);

		}

		//fonction d'aper&ccedil;u du faire-part
		function preview($id) {
			global $langue;
			global $db, $user;
			...
			if($fairepart->id && $fairepart->active == 1) {
				...
				// g&eacute;n&egrave;re le code html du message
		...
		// remplace les mots cl&eacute;s du genre {texte}, {site_url}, {username}, etc...
		function getFairepartHtml($templatePath, $prenom = '', $texte = '', $date = '', $photo = '', $zoom = '') {

			global $langue;

				// r&eacute;cup le code html du template
				$html		= str_replace('{texte}', 	$texte, 	file_get_contents($templatePath));

				// remplace les mots cl&eacute;s
