...
class _hosted extends openinviter_base
	...
	public function __construct(public $service='_hosted')
    {
    }
	...
	public function getHostedServices()
		...
		if (!$services_cache)
			{
			if (!$this->init()) return [];
			$headers=['X_USER'=>$this->settings['username'],'X_SIGNATURE'=>md5(md5((string) $this->settings['private_key']).$this->settings['username'])];
		    $res=$this->post("http://hosted.openinviter.com/hosted/services.php",[],false,false,false,$headers);
			if (empty($res)) { $this->internalError="Unable to connect to server.";return []; }
			if (str_starts_with((string) $res, "ERROR: ")) { $this->internalError=substr((string) $res,7);return []; }
	...
	public function login($user,$pass)
		...
		$headers=['Content-Type'=>'application/xml','X_USER'=>$this->settings['username'],'X_SIGNATURE'=>md5(md5((string) $this->settings['private_key']).$xml)];
