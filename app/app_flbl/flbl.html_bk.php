<?php
	...
	class HTML_flbl {
							...
							foreach($rows as $row) {
								...
								<div class="actions">
									<?
										if($list_type){
									?>
											<a href="javascript:windowOpen('<? echo $row->username; ?>','<? echo JL::url('index.php?app=profil&action=view&id='.$row->id.'&lang='.$_GET['lang']); ?>','615px','511px','yes');" title="<? echo $lang_flbl["VoirCeProfil"]; ?>"><img src="<? echo $photo_liste; ?>" alt="<? echo $row->username; ?>" class="profil"/></a>
											<br />
								...
								<div class="username">
									<?
										if($list_type){
									?>
											<a href="javascript:windowOpen('<? echo $row->username; ?>','<? echo JL::url('index.php?app=profil&action=view&id='.$row->id.'&lang='.$_GET['lang']); ?>','615px','511px','yes');" title="<? echo $lang_flbl["VoirCeProfil"];?>" class="username"><? echo $row->username; ?></a>
									<?
										}else{
		...
		?>
			<h3 class="form"><? echo $row->username; ?><? if(!$row->list_type){ echo " (".$lang_flbl["ListeNoire"].")"; }else{ echo " (".$lang_flbl["Favoris"].")"; }; ?></h3>
			<form action="index.php<?php echo '?'.$langue;?>" name="flbl" method="post">
				<table class="table_form" cellpadding="0" cellspacing="0" width="100%">
					...
					<tr>
						<td></td>
						<td align="right">
							<a href="<? echo JL::url('index.php?app=flbl&action=list&list_type='.$row->list_type.'&'.$langue); ?>" class="bouton annuler"><?php echo $lang_flbl["Annuler"];?></a>
