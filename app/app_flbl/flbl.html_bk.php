<?php
	...
	class HTML_flbl {
		...
		function flbl_titre($list_type) {
			...
			?>
				<h3 class="result"><?php echo $h1; ?></h3>
			<?php 			/*}*/
						...
						if(is_array($rows) && $nb_rows) {
							...
							foreach($rows as $row) {
								...
								<div class="actions">
									<?php 										if($list_type){
									?>
											<a href="javascript:windowOpen('<?php echo $row->username; ?>','<?php echo JL::url('index.php?app=profil&action=view&id='.$row->id.'&lang='.$_GET['lang']); ?>','615px','511px','yes');" title="<?php echo $lang_flbl["VoirCeProfil"]; ?>"><img src="<?php echo $photo_liste; ?>" alt="<?php echo $row->username; ?>" class="profil"/></a>
											<br />
								...
								<div class="username">
									<?php 										if($list_type){
									?>
											<a href="javascript:windowOpen('<?php echo $row->username; ?>','<?php echo JL::url('index.php?app=profil&action=view&id='.$row->id.'&lang='.$_GET['lang']); ?>','615px','511px','yes');" title="<?php echo $lang_flbl["VoirCeProfil"];?>" class="username"><?php echo $row->username; ?></a>
									<?php 										}else{
		...
		?>
			<h3 class="form"><?php echo $row->username; ?><?php if(!$row->list_type){ echo " (".$lang_flbl["ListeNoire"].")"; }else{ echo " (".$lang_flbl["Favoris"].")"; }; ?></h3>
			<form action="index.php<?php echo '?'.$langue;?>" name="flbl" method="post">
				<table class="table_form" cellpadding="0" cellspacing="0" width="100%">
					...
					<tr>
						<td></td>
						<td align="right">
							<a href="<?php echo JL::url('index.php?app=flbl&action=list&list_type='.$row->list_type.'&'.$langue); ?>" class="bouton annuler"><?php echo $lang_flbl["Annuler"];?></a>
							<a href="javascript:document.flbl.submit();" class="bouton envoyer"><?php echo $lang_flbl["Valider"];?></a>
						</td>
					</tr>
				</table>
