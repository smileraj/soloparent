<?php
	...
	class HTML_flbl {
		...
		// titre
		public static function flbl_titre($list_type) {
			global $langue;
			include("lang/app_flbl.".$_GET['lang'].".php");
			global $user, $action;
		...
		public static function flblList(&$rows, &$messages, $list_type) {
			global $langue;
			include("lang/app_flbl.".$_GET['lang'].".php");
			...
			<?
						...
						if(is_array($rows) && $nb_rows) {
							...
							foreach($rows as $row) {
								...
								?>
								<div class="col-md-6 parentsolo_pt_15 parentsolo_pb_15 nopadding">
									<div class="member_box">
						
						<div class="col-md-4 col-xs-12 col-sm-3 parentsolo_pt_10">
							<div class="hovereffect parentsolo_border_radius  ">
								<?
										if($list_type){
									?>
									<a href="<? echo JL::url('index.php?app=profil&action=view&id='.$row->id.'&lang='.$_GET['lang']); ?>"  title="<? echo $lang_flbl["VoirCeProfil"]; ?>">
									<img src="<? echo $photo_liste; ?>" alt="<? echo $row->username; ?>" class="profil"/></a>
									<?
										}else{
									?>
						...
						<div class="col-md-8 col-xs-12 col-sm-9 parentsolo_pt_10 parentsolo_pb_10">
								<div class="col-xs-11 nopadding"><h4 class="letter_spacing_0  parentsolo_pb_10 parentsolo_font-size parentsolo_txt_overflow_title">
								<?
										if($list_type){
									?>
						...
						</h4></div><div class="col-xs-1 nopadding"><a href="javascript:if(confirm('<?php echo $list_type == 1 ? $lang_flbl["ConfirmationRetraitFavoris"] : $lang_flbl["ConfirmationRetraitListeNoire"] ; ?> ?')){document.location='<? echo JL::url('index.php?app=flbl&action=remove&list_type='.$list_type.'&id='.$row->id.'&'.$langue); ?>';}" title="<?php echo $list_type == 1 ? $lang_flbl["RetraitFavoris"] : $lang_flbl["RetraitListeNoire"] ; ?>"><img src="<? echo SITE_URL.'/images/non.gif'; ?>" alt="<?php echo $list_type == 1 ? $lang_flbl["RetraitFavoris"] : $lang_flbl["RetraitListeNoire"] ; ?>" /></a></div>
						<div class="clear"></div>
						...
						<div class="infos"><? echo $row->age; ?> <?php echo $lang_flbl["ans"];?> - <? echo $row->nb_enfants; ?> <? echo $row->nb_enfants > 1 ? $lang_flbl["enfants"] : $lang_flbl["enfant"]; ?> - <? echo $row->canton_abrev; ?> <? if($list_type){ ?><div class="connect"><span class="<? echo $last_online_class; ?>"><? echo $last_online_label; ?></span></div><? } ?>
						
						</div>
						<div class="supplement">
									<b><?php echo $lang_flbl["AjouteLe"];?></b> - <? echo date('d/m/Y', strtotime($row->datetime_add)); ?> </div>
								<div class="connect">
								<a href="<? echo JL::url('index.php?app=flbl&action=add&list_type='.$list_type.'&user_id_to='.$row->id.'&'.$langue); ?>" title="<? echo $lang_flbl["ModifierLeCommentaire"];?>"><? echo $lang_flbl["ModifierLeCommentaire"];?></a>
						</div>	
						</div>
						...
						<div class="col-md-12 parentsolo_pt_10">
								<div class="description text-box" style="height:20px; overflow:hidden;">
								...
								<div class="actions">
									<?
										if($list_type){
									?>
											<a href="<? echo JL::url('index.php?app=profil&action=view&id='.$row->id.'&lang='.$_GET['lang']); ?>" target="_blank" title="<? echo $lang_flbl["VoirCeProfil"]; ?>"><img src="<? echo $photo_liste; ?>" alt="<? echo $row->username; ?>" class="profil"/></a>
											<br />
											<a href="<? echo JL::url('index.php?app=message&action=write&user_to='.$row->username.'&'.$langue); ?>" title="<?php echo $lang_flbl["EnvoyerUnMail"];?>"><img src="<? echo SITE_URL; ?>/<? echo SITE_TEMPLATE; ?>/images/btn_message.png" alt="<?php echo $lang_flbl["EnvoyerUnMail"];?>" /></a>
											<a href="<? echo JL::url('index.php?app=message&action=flower&user_to='.$row->username.'&'.$langue); ?>" title="<?php echo $lang_flbl["EnvoyerUneRose"];?>" ><img src="<? echo SITE_URL; ?>/<? echo SITE_TEMPLATE; ?>/images/btn_rose.png" alt="<?php echo $lang_flbl["EnvoyerUneRose"];?>" /></a>
											<a href="javascript:windowOpen('ParentSoloChat','<? echo JL::url('index.php?app=chat&id='.$row->id.'&'.$langue); ?>','800px','600px');" title="<?php echo $lang_flbl["Chat"];?>"><img src="<? echo SITE_URL; ?>/<? echo SITE_TEMPLATE; ?>/images/btn_chat.png" alt="<?php echo $lang_flbl["Chat"];?>" /></a>
								...
								<div class="username">
									<?
										if($list_type){
									?>
											<a href="<? echo JL::url('index.php?app=profil&action=view&id='.$row->id.'&lang='.$_GET['lang']); ?>" target="_blank" title="<? echo $lang_flbl["VoirCeProfil"];?>" class="username"><? echo $row->username; ?></a>
									<?
										}else{
		...
		// ajout/édition
		public static function flblAdd(&$row, &$messages) {
			global $langue;
			include("lang/app_flbl.".$_GET['lang'].".php");
