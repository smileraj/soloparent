<?php
	...
	switch($action) {
		...
		case 'methode':
			$methode_id		= (int)JL::getVar('methode_id', 5);
			...
			if($methode_id == 1) {
				...
				if($custom == '') {
					...
					abonnementRazarPayForm($abonnement_id);

				} else {
					abonnementRazarPayReturn($custom);
	...
	function abonnementRazarPayForm($abonnement_id) {
		global $langue;
		global $db, $user;
		...
		// tarifs de l'abonnement d&eacute;sir&eacute;
		...
		if(!$abo){
			...
			$query = "INSERT INTO abonnement_razarpay SET"
			." user_id = '".$user->id."',"
			." date_enregistrement = '".date('Y-m-d H:i:s', $time)."',"
			." date_souscription = '',"
			." date_dernier_renouvellement = '',"
			." date_annulation = '',"
			." valide = 0," //0 = non valid&eacute;, 1 = en cours, 2 = annul&eacute;
			." intitule_abo = '".$row->nom."',"
			." montant = '".$row->montant."',"
			." duree_razarpay = '".$row->duree_paypal."',"
			." unite_duree_razarpay = '".$row->unite_duree_razarpay."',"
			." nb_paiement = 0"
			;
			$db->query($query);
			$row->razarpay_id = $db->insert_id();


			// g&eacute;n&eacute;ration de la valeur de retour 'custom'
			$row->custom 	= base64_encode(MD5($user->username.$user->gid.date('Y-m-d H:i:s', $time)).":".$row->razarpay_id);

//
global $langue;
$api_key = 'rzp_test_V7m4qbjpIX99bi';
$api_secret = 'sAzNfkx3tcRtSK3cy8INvywu';
var_dump($api_key, $api_secret);
...
$data = [
  ...
  "callback_url" => "http://localhost/index.php?app=abonnement&action=methode&custom=".$row->custom."&".$langue,
  "callback_method" => "get"
];
...
//
	...
	function abonnementRazarPayReturn() {
		global $langue, $db;
	
		$razorpay_payment_id = isset($_GET['razorpay_payment_id']) ? $_GET['razorpay_payment_id'] : null;
		$razorpay_payment_link_id = isset($_GET['razorpay_payment_link_id']) ? $_GET['razorpay_payment_link_id'] : null;
		$razorpay_payment_link_status = isset($_GET['razorpay_payment_link_status']) ? $_GET['razorpay_payment_link_status'] : null;
		$razorpay_signature = isset($_GET['razorpay_signature']) ? $_GET['razorpay_signature'] : null;
		$custom_encoded = isset($_GET['custom']) ? $_GET['custom'] : null;
	
		if (!$razorpay_payment_id || !$razorpay_payment_link_id || !$razorpay_payment_link_status || !$razorpay_signature || !$custom_encoded) {
			die("Invalid payment data.");
		}
	
		$api_secret = 'sAzNfkx3tcRtSK3cy8INvywu';

		$data = $razorpay_payment_link_id . '|' . $razorpay_payment_id;
		$expected_signature = hash_hmac('sha256', $data, $api_secret);
		...
		// Decode custom data
		$arguments = explode(":", base64_decode($custom_encoded));
		$md5 = $arguments[0];
		$razorpay_id = isset($arguments[1]) ? (int)$arguments[1] : 0;
	
		// Fetch subscription duration info from DB
		$query = "SELECT duree_razarpay AS duree, unite_duree_razarpay AS unite FROM abonnement_razarpay WHERE id = '" . intval($razorpay_id) . "' LIMIT 1";
		$duree_abo = $db->loadObject($query);
	
		// Call post-payment processing function
		HTML_abonnement::razorPayReturn($duree_abo);
	}
	...
	function abonnementPaypalReturn($custom) {
		...
		// r&eacute;cup la dur&eacute;e d'abo
...
$query = "INSERT INTO postfinance SET"
	...
	$server			= 'ssl://www.paypal.com';
				...
				if($txn_type == 'Success'){
					// r&eacute;cup les d&eacute;tails du paiement
					$query = "SELECT *"
					." FROM abonnemnet_postfinance"
					...
					// r&eacute;cup les d&eacute;tails de l'utilisateur
					...
					//if($abonnement_paypal && $userProfil && $mc_currency == $currency_check && $mc_gross == $abonnement_paypal->montant){
						...
						// met &agrave; jour le status de la demande de transaction
						...
						// met &agrave; jour la date limite d'abonnement
...
$confrm_subject="Parentsolo.ch Payment Details";
	...
	$confrm_subject="Parentsolo.ch Betalings details";
		...
		<b>Om technische redenen kan de beëindiging van het abonnement niet worden gemaakt van 24 uur na de aankoop van het abonnement en 48 uur voor het verstrijken van het huidige abonnement, tenzij u een aanbieding krijgt test.</b>  
		</p></td></tr>
		</table>";
	...
	$confrm_subject="Parentsolo.ch Détails de paiement";
		...
		<strong >Pay Id </strong>: ".$PAYID."<br /><br />
		<strong >R&eacute;f&eacute;rence de commande </strong>: ".$customerid."<br/><br />
		<strong >Abonnement </strong>: ".$abonnement_paypal->intitule_abo."<br /><br />
		<strong >Paiement </strong>: CHF ".$currency."<br />Date limite d'abonnement changée de ".$userProfil->gold_limit_date." &agrave; ".$gold_limit_date."<br /><br />".$points."<br /><br />
