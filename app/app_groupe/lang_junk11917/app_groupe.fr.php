<?php

$lang_groupe = array(
"upload_file" => "Parcourir",

	//------------------Page groupe.html.php ---------

	"MessagesParentsolo" => "Messages solocircl.com",
	
	"MesGroupes" => " Mes groupes",
	"General" => "G&eacute;n&eacute;ral",
	"Informations" => "Informations",
	
	"TousLesGroupes" => "Tous les groupes",
	"GroupesRejoints" => "Groupes rejoints",
	"MesCreations" => "Mes cr&eacute;ations",
	"CreerUnGroupe" => "Cr&eacute;er un groupe",
	"Groupe" => "Groupe",
	"CodeDeVerification" => "Code de v&eacute;rification",
	"VeuillezRecopierCodeVerification" => "Veuillez recopier ci-dessous, ce code de v&eacute;rification:",
	"Recherche" => "Recherche",
	"OrdonnerPar" => "Ordonner par",
	"Rechercher" => "Rechercher",
	
	
	
	"Refuse" => "Refus&eacute;",
	"Valide" => "Valid&eacute;",
	"Validation" => "En cours de validation",
	"Verouille" => "Verouill&eacute;",
	
	
	"ModifierCeGroupe" => "Modifier ce groupe",
	"DetailsDuGroupe" => "D&eacute;tails du groupe",
	
	"Nouveau" => "Nouveau",
	
	
	"AucunMembre" => "Aucun membre n'a encore rejoint ce groupe",
	"MembreARejointCeGroupe" => "membre a rejoint ce groupe",
	"MembresOntRejointCeGroupe" => "membres ont rejoint ce groupe",
	
	"Membre" => "Membre",
	
	"ConfirmationQuitterGroupe" => "Voulez-vous vraiment quitter ce groupe",
	"Quitter" => "Quitter",
	"ConfirmationRejoindreGroupe" => "Voulez-vous vraiment rejoindre ce groupe",
	"Rejoindre" => "Rejoindre",
	
	
	"PagePrecedente" => "Page pr&eacute;c&eacute;dente",
	"Pages" => "Pages",
	"Page" => "Page",
	"Debut" => "D&eacute;but",
	"Fin" => "Fin",
	"groupe" => "groupe",
	"groupes" => "groupes",
	"PageSuivante" => "Page suivante",
	
	
	"AucunGroupe" => "Aucun groupe ne correspond &agrave; votre recherche",
	
	
	"Popularite" => "Popularit&eacute;",
	
	
	"Fondateur" => "Fondateur",
	
	"PapasOntRejointCeGroupe" => "papas ont rejoint ce groupe",
	"PapaARejointCeGroupe" => "papa a rejoint ce groupe",
	"MamansOntRejointCeGroupe" => "mamans ont rejoint ce groupe",
	"MamanARejointCeGroupe" => "maman a rejoint ce groupe",
	
	"EnLigne" => "En ligne",
	"HorsLigne" => "Hors ligne",
	
	
	"VoirCeProfil" => "Voir ce profil",
	
	
	
	"Ans" => "ans",
	"Enfants" => "enfants",
	"Enfant" => "enfant",
	
	
	"membres" => "membres",
	"membre" => "membre",
	
	
	"ContenuVuParAutreMembre" => "Contenu vu par les autres membres",
	
	
	"DateDeCreation" => "Date de cr&eacute;ation",
	"A" => "&agrave;",
	
	"Message" => "Message",
	
	
	
	"GroupeNonVisibleEnCoursDeValidation" => "Votre groupe n'est pas visible par les autres membres car il est en cours de validation et qu'il n'a jamais &eacute;t&eacute; valid&eacute; par le pass&eacute;",
	"GroupeNonVisibleRefuse" => "Votre groupe n'est pas visible par les autres membres car il a &eacute;t&eacute; refus&eacute;",
	
	"ModifierMonGroupe" => "Modifier mon groupe",
	"CreerMonGroupe" => "Cr&eacute;er mon groupe",
	
	
	"Statut" => "Statut",	
	"Titre" => "Titre",
	"Texte" => "Texte",
	
	"WarningCodeVerifIncorrect" => "Le code de v&eacute;rification entr&eacute; est incorrect",
	
	"EnvoyerPhoto" => "Envoyer Photo",
	
	
	"Supprimer" => "Supprimer",
	
	
	
	"PhotoEnAttenteValidation" => "Cette photo est en attente de validation par un mod&eacute;rateur de solocircl.com <br/><br/>Tant que la nouvelle photo sera en attente de validation, l'ancienne photo sera toujours utilis&eacute;e",
	
	
	"ConfirmationAnnulation" => "Voulez-vous vraiment annuler la cr&eacute;ation de ce groupe",
	"Annuler" => "Annuler",
	
	"Envoyer" => "Envoyer",	
	
	
	
	
	
	
	
	"Nouveaute" => "Nouveaut&eacute;",
	"Anciennete" => "Anciennet&eacute;",
	"OrdreAlphabetique" => "Ordre alphab&eacute;tique",
	"NombreMembres" => "Nombre de membres",
	
	
	
	
	
	
	"RejoindreGroupeReussi" => "Vous avez rejoint le groupe",
	
	"QuitterGroupeReussi" => "Vous avez quitt&eacute; le groupe",
	
	"GroupeEnregistre" => "Groupe enregistr&eacute;",
	"GroupesSoumisAValidation" => "Toute cr&eacute;ation ou modification d'un groupe est v&eacute;rifi&eacute;e par un mod&eacute;rateur de solocircl.com avant d'&ecirc;tre publi&eacute; ou non sur le site",
	"AffichageGroupeDansListe" => "Votre nouveau groupe ou les modifications apport&eacute;es &agrave; celui-ci seront visibles dans l'onglet \"Tous les groupes\" apr&egrave;s validation par ce mod&eacute;rateur",
	
	"NonFondateurGroupe" => "Vous ne pouvez pas modifer ce groupe car vous n'&ecirc;tes pas le fondateur de celui-ci",
	
	"GroupeVerouille" => "Ce groupe a &eacute;t&eacute; verouill&eacute; par un mod&eacute;rateur de solocircl.com, vous ne pouvez donc plus le modifier",
	
	"RejoindreGroupeInexistant" => "Le groupe que vous tentez de rejoindre n'existe pas ou plus",
	
	"QuitterGroupeInexistant" => "Le groupe que vous tentez de quitter n'existe pas ou plus",
	
	"GroupeInexistant" => "Le groupe n'existe pas ou plus",
	
	
	
	"VerificationModificationsGroupe" => "Toutes les modifications effectu&eacute;es sur un groupe devront &ecirc;tre v&eacute;rifi&eacute;es par un mod&eacute;rateur de solocircl.com avant d'&ecirc;tre publi&eacute;es ou non sur le site",
	"VerificationCreationGroupe" => "Votre groupe devra &ecirc;tre v&eacute;rifi&eacute; par un mod&eacute;rateur de solocircl.com avant d'&ecirc;tre publi&eacute; ou non sur le site",
	
	
	"IndiquezTitre" => "Veuillez indiquer le titre de votre groupe, s'il vous pla&icirc;t",
	
	"TitreExistant" => "Un groupe portant le m&ecirc;me titre existe d&eacute;j&agrave;. Veuillez modifier le titre de votre groupe, s'il vous pla&icirc;t",
	
	"IndiquezDescription" => "Veuillez indiquer la description de votre groupe, s'il vous pla&icirc;t",

	
);

?>
