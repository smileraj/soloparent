<?php
	...
	class fairepartView extends JLView {
		...
		function listall(&$rows, &$messages, &$list, &$pagination) {
			...
			if(!$user->id){ //article pour les conditions des castings et si utilisateur n'est pas identifi�
			?>
				<p>
			...
			<?php 			}else{
				...
				// toolbar
				JLToolbar::open();
				JLToolbar::link(JL::url('index.php?app=membre&'.$langue), 'Fermer', 'Retour � la liste des faire-parts', 'annuler');
				JLToolbar::link(JL::url('index.php?app=fairepart&action=edit&'.$langue), 'Nouveau', 'Cr�er un nouveau faire-part', 'nouveau');
				JLToolbar::close();
		...
		function edit(&$row, &$messages, &$list, &$destinataire) {
			...
			JLToolbar::link(JL::url('index.php?app=fairepart&'.$langue), 'Fermer', 'Retour � la liste des faire-parts', 'annuler');
			...
			// si on �dite un faire part existant
			if($row->id) {
				JLToolbar::link(JL::url(SITE_URL.'/index.php?app=fairepart&action=preview&id='.$row->id.'&template=fairepart&'.$langue), 'Aper�u', 'Voir un aper�u du faire-part', 'apercu', true);
			}
		...
		//affiche la page d'erreur si le faire part est d�sactiv�
		function error404(){
			global $langue;
