<?php
	...
	/*
		...
		reglement: affiche le r�glement(content)
		cadeaux: liste des lots(content)
		bareme: affiche le bareme
		mespoints: liste le d�tail des points de l'utilsiateur log
		classement: classement actuel en temps r�el
		archives: archives des pr�c�dents mois
	*/
	switch($action) {
	...
	function pointsTemoignage($id) {
		...
		// r�cup le dernier t�moignage en date du syst�me de points
	...
	// liste les t�moignages
	function pointsTemoignages() {
			global $langue;
		global $db;

		// r�cup les derniers t�moignages (todo + tard: pagination)
	...
	// bar�me du syst�me de points
	function pointsBareme() {
			global $langue,$langString;
		global $db;

		// r�cup le bareme, ordre par nombre de points DESC
	...
	// points gagn�s par l'utilisateur log
...
//die($langString);
	...
	function pointsClassement() {
		...
		// r�cup les 20 premiers du classement actuel
		...
		for($i=0; $i<$count; $i++) {
			...
			// classement pr�c�dent date de 2 jours, on met � jour avec le nouveau
			if($rows[$i]->last_rank_days >= 2) {

				// classement trop ancien, membre �ject� du classement et revient en top 20
				if($rows[$i]->last_rank_days > 2) {

					// r�initialise son rang pr�c�dent
					$rows[$i]->last_rank = 0;
					$last_rank = 0;
	...
	function pointsArchives() {
		...
		// r�cup les 3 derniers classements, soit 3*10 membres
