<?php
	...
	function flblList($list_type) {
		...
		// r�cup les users en friend/black list
		...
		$added		= (int)JL::getVar('added', 0, true); 	// id de l'utilisateur ajout� � $list_type
		$removed	= (int)JL::getVar('removed', 0, true); 	// id de l'utilisateur retir� de $list_type
	...
	function flblAdd() {
		...
		if($user_to->id) {
			...
			// v�rifie si l'utilisateur cible est d�j� dans la FL/BL
	...
	function flblSave() {
		...
		if($row->user_id_to) {
			...
			// supprime l'enregistrement existant (permet de passer de BL � FL et inversement � coup s�r)
