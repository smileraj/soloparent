...
class openinviter
	...
	public function startPlugin($plugin_name,$getPlugins=false)
		...
		elseif ($this->settings['hosted'])
			{
			if (!file_exists($this->basePath."/plugins/_hosted.plg.php")) $this->internalError=$lang_appexample["WarningBoiteOuReseauInvalide"];
			else
				{
		...
		elseif (file_exists($this->basePath."/plugins/{$plugin_name}.plg.php"))
			...
			if (file_exists($this->basePath."/conf/{$plugin_name}.conf")) 
				...
				if (empty($enable)) $this->internalError=$lang_appexample["WarningBoiteOuReseauInvalide"];
		...
		else { $this->internalError=$lang_appexample["WarningBoiteOuReseauInvalide"];return false; }
		return true;
		}
	...
	private function checkLoginCredentials($user)
		...
		if ($this->currentPlugin['requirement'])
			...
			if ($this->currentPlugin['requirement']=='email' AND !$is_email)
				{
				$this->internalError=$lang_appexample["WarningEntrerEmailEntier"];
				return false;
				}
			elseif ($this->currentPlugin['requirement']=='user' AND $is_email)
				{
				$this->internalError=$lang_appexample["WarningEntrerJusteUsername"];
				return false;
				}
			}
		if ($this->currentPlugin['allowed_domains'] AND $is_email)
			...
			if (!$temp)
				{
				$this->internalError="<b>{$user_domain}</b> ".$lang_appexample["WarningMauvaisNomDomaine"];
				return false;
				}
			}
		return true;
