<?php
	...
	function messageRead($id) {
		...
		// d�termine si c'est un message envoy� ou re�u
		...
		} elseif($message->user_id_from == $user->id) { // message envoy�

			$innerJoinOn	= 'm.user_id_to';

		} else {

			// sinon c'est que le message n'appartient pas � l'utilisateur log
			JL::redirect(SITE_URL.'/index.php?app=message&action=inbox'.'&'.$langue);
		...
		$where[]	= "u.confirmed > 0";
		//$where[]	= "u.published = 1";
		// pourquoi published 0 autoris� ? car on permet l'affichage de messages de profils d�sactiv�s, mais confirm�s.
		// par contre, on n'affiche pas les messages de profils non confirm�s ! qu'ils soient publi�s ou non
		...
		// r�cup le message de l'utilisateur log
		...
		if($message->non_lu && $message->user_id_to == $user->id) {
			...
			// met � jour le non_lu du message
			$query = "UPDATE message SET non_lu = 0 WHERE id = '".$id."'";
			$db->query($query);

			// mise � jour de user_stats du destinataire
			if($message->fleur_id) {
	...
	function messageList($dossier_id, $flowers = false) {
		...
		// r�cup les messages de l'utilisateur log
	...
	function messageSent() {
		...
		// r�cup les messages envoy�s par l'utilisateur log
	...
	function messageReply($id, $messages = array()) {
		...
		// message n'appartenant pas � l'utilisateur
		if($message->user_id_to != $user->id) {
		...
		// affecte les param�tres par d�faut
	...
	function messageSend($flower = false) {
		...
		// v�rification du formulaire avant envoi
		...
		// r�cup le status de l'utilisateur (confirm�, attente, non confirm�)
		$query = "SELECT confirmed FROM user WHERE id = '".(int)$user->id."' LIMIT 0,1";
		$user->confirmed = $db->loadResult($query);


		// si l'utilisateur est en attente de confirmation t�l�phonique
		if($user->confirmed == 2) {
		...
		if(!$user_to) {
			...
			// v�rifie si le destinataire existe dans la DB
		...
		if (is_array($messages)) {
			...
			if($fleur_id) {
				...
				// cr�dite l'action r�ception de fleur
				...
				// cr�dite l'action r�ception de mail
				JL::addPoints(8, $user_to->id, $user_to->id.'#'.$user->id.'#'.date('d-m-Y'));

			}
			$db->query($query);

			// mise � jour de user_stats de l'exp�diteur
			if($fleur_id) {
		...
		if(is_array($ids)) {
			...
			foreach($ids as $id) {
				...
				if($message->user_id_to != $user->id || !in_array($dossier_id, $dossier_id_ok)) {
					...
					// met � jour le dossier_id
					$query = "UPDATE message SET dossier_id = '".$dossier_id."' WHERE id = '".$id."'";
					$db->query($query);

					// nouveau message, non lu, destin� � l'utilisateur log
					if($message->non_lu == 1 && $message->user_id_to == $user->id) {
						...
						if($dossier_id == 1 && $message->dossier_id == 0) {
							...
							// d�cr�mente le compteur de nouveaux messages
							$op = '-';

						} elseif($dossier_id == 0 && $message->dossier_id == 1) { // restauration

							// incr�mente le compteur de nouveaux messages
							$op = '+';

						}

						// s'il y a une op�ration � faire
						if($op != '') {

							// d�termine le champ � utiliser
							$field	= $message->fleur_id > 0 ? 'fleur_new' : 'message_new';

							// met � jour les stats de messages non lus
			...
			// on affichera un message d'erreur apr�s la redirection
			$msg = 0;
