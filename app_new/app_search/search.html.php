...
class HTML_search {
	...
	function search(&$list, &$results, &$messages) {
		global $langue;
		include("lang/app_search.".$_GET['lang'].".php");
		global $action;
...
<span><br><?php echo JL::calcul_age($result->naissance_date); ?><br />
	...
	function searchVisits(&$list, &$results, &$messages) {
		global $langue;
		include("lang/app_search.".$_GET['lang'].".php");
		global $action;
	...
	function searchOnline(&$list, &$results, &$messages) {
		global $langue;
		include("lang/app_search.".$_GET['lang'].".php");
		global $action;
	...
	function searchLastInscription(&$list, &$results, &$messages) {
		global $langue;
		include("lang/app_search.".$_GET['lang'].".php");
		global $action, $user;
	...
	function profile_matching(&$list, &$results, &$messages) {
		global $langue;
		include("lang/app_search.".$_GET['lang'].".php");
		global $action, $user;
	...
	/* affiche le formulaire d'un crit�re facultatif
		$field: nom du champ html
		$title: intitul� du crit�re
	...
	*/
	function searchFormCritereFacultatif($field, $title, &$lists) {
		...
		// variables
		$list		= $lists[$field];
		$tdParLigne = 6;	// nombre de crit�res facultatifs par ligne
		...
		?>
			<table class="table_search2" cellpadding="0" cellspacing="0">
				...
				<?php 
					// d�termine les it�rations � effectuer
