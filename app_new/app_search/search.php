<?php
	...
	// si non log dans l'appli search (hors step6submit qui fait partie du processus compliqu� d'inscription)
...
//end profile Matching
	...
	function searchSave() {
		...
		// donn�es du formulaire de recherche
		$_data	= search_data();


		// r�cup les donn�es en session
		if (is_array($_data)) {
		...
		if(!$search['search_titre']) {
			$search['search_titre'] = ''.$lang_search["MaRechercheDu"].' '.date('d/m/Y').' � '.date('H:i');
		}

		// user log
		if($user->id) {

			// supprime l'ancienne recherche du m�me nom si elle existe
	...
	// variables accept�es par le moteur de recherche
	...
	function searchResults() {
		...
		// r�cup les variables qui peuvent �tre pass�es directement dans l'url
		
		// pagination
		$resultatParPage	= 16;
		

		// correction au cas o� le visiteur s'amuserait avec les params de l'url
	...
	// r�cup�ration des visites d'un profil
...
$age_max_pro=(($values_select->userage)+5);
	...
	function searchSubmit() {
		...
		// stock les donn�es temporaires en session
		if (is_array($_data)) {
			foreach($_data as $key => $value) {
...
// ville
		...
		// crit�res facultatifs
			...
			." ORDER BY nom_".$_GET['lang']." ASC"
			;
			$dbnationalit�=$db->loadObjectList($query);
			$list['search_nationalite_id'] = JL::makeCheckboxSearchList($dbnationalit�, 'search_nationalite_id[]', '', 'value', 'text', $row['search_nationalite_id']); 
			...
			." ORDER BY nom_".$_GET['lang']." ASC"
			;
			$dbtemp�rament=$db->loadObjectList($query);
			$list['search_temperament_id'] = JL::makeCheckboxSearchList($dbtemp�rament, 'search_temperament_id[]', '', 'value', 'text', $row['search_temperament_id']); 
	...
	// cr�ation de la clause WHERE d'un crit�re facultatif
