<?php
	...
	// types de groupes: tous, que ceux rejoinds par l'utilisateur, que ceux cr��s par l'utilisateur
	...
	function groupeJoin($groupe_id) {
		...
		if($id > 0) {
			...
			// check si l'utilisateur est d�j� inscrit
			...
			// si l'utilisateur n'est pas d�j� inscrit (s'il l'est, on n'enregistre rien, mais pas besoin de g�n�rer un message d'erreur)
			if(!$dejaInscrit) {
	...
	function groupeInfo() {
		...
		// todo: cr�er un article et le charger depuis la fonction
	...
	function groupeList($groupe_type) {
		...
		// formule magique: calcule de la popularit� d'un groupe
		...
		switch($groupe_type) {
			...
			case 'created': // groupes cr��s par l'utilisateur
		...
		// r�cup le score de popularit� max (on n'utilise pas le $_where car on calcule le score g�n�ral, pour tous les groupes valides, et non pour la recherche uniquement)
		...
		// cr�dit de points pour les seuils de popularit� atteints pour la premi�re fois
		if(is_array($rows)) {
			foreach($rows as $row) {

				if($row->popularite < 50) { // 2 ic�nes
					JL::addPoints(14, $row->user_id, $row->id);
				} elseif($row->popularite < 75) { // 3 ic�nes
					JL::addPoints(15, $row->user_id, $row->id);
				} elseif($row->popularite < 100) { // 4 ic�nes
					JL::addPoints(16, $row->user_id, $row->id);
				} elseif($row->popularite == 100) { // 5 ic�nes
		...
		} elseif($msg == 'locked') { // groupe verouill�
	...
	function groupeEdit(&$messages) {
		...
		// r�cup les donn�es du formulaire
		$row	= new stdClass();
		$_data 	=& groupe_data();
		...
		if($row->id) {
			...
			} elseif($rowTemp->active == 3) { // groupe non verouill�
	...
	// ajoute un utilisateur � la fl/bl de l'utilisateur log
	function &groupeSave() {
			global $langue;
			include("lang/app_groupe.".$_GET['lang'].".php");
		global $db, $user;
 ...
 $groupe->captchaAbo			= JL::getVar('captchaAbo', '');
	 $groupe->verif					= trim(JL::getVar('verif', ''));
		...
		// check que le groupe appartient bien � l'utilisateur log
		if($row->id) {
			...
			} elseif($groupe->active == 3) { // groupe verrouill�
		...
		if(!$row->titre) {
			...
			// v�rifie si un groupe du m�me titre existe d�j�
		...
		if(!count($messages)) {
			...
			// reset le statut du groupe, en le d�finissant comme "� valider"
			$query .= " active = 2,"
			...
			if(!$row->id) {
				...
				// le cr�ateur du groupe rejoind automatiquement son propre groupe
				$query = "INSERT INTO groupe_user SET"
				." groupe_id = '".$row->id."',"
				." user_id = '".(int)$user->id."'"
				;
	...
	function groupeFiche($id) {
		...
		if(JL::getVar('page_h', 1) == 1 && JL::getVar('page_f', 1) == 1) {
			...
			// mise � jour des stats de visites du groupe
