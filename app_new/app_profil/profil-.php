<?php
	...
	if(in_array($action, ['panel', 'step8', 'step8submit'])) {
	...
	switch($action) {
		...
		case 'step9':
			step_check(9); // on peut aller directement � la fin de l'inscription, mais il faut avoir valid� l'�tape 1
			step9();
		break;
	...
	function panel() {
		...
		// connexion al�atoire de nos profils entre 8 et 24 heures
		/*if(date('G') >= 8 AND rand(0,10) == 0) {
			...
			/
				...
				profils � exclure des connexions automatiques
				> jolito 		672
				> pauline33		22
				> benji		21
				> nanette		19
	...
	function mdpsubmit() {
		...
		// Mdp confirmation
		if($_data['password2'] && !preg_match('/^[a-zA-Z0-9._-]+$/', (string) $_data['password2'])) {
		...
		if(!count($messages)) {
			...
			// envoi du mail
			mail((string) $_data['email'], $titre, $texte, $headers);
	...
	function step_check($step_num) {
		...
		if(!$user->id) {
			...
			if($step_num > 1 && JL::getSessionInt('step_ok', 0) < 1) { // page inscription obligatoire (�tape 1)
				JL::redirect("index.php?app=profil&action=inscription".'&'.$langue);
			} elseif($step_num > 9 && JL::getSessionInt('step_ok', 0) < 9) { // page coordonn�es obligatoire (�tape 9)
				JL::redirect("index.php?app=profil&action=step9".'&'.$langue);
			} else {
	...
	function step1($messages = []) {
		...
		$list_canton_id				= [];
		$list_ville_id				= [];
		$list_recherche_age_min		= [];
		$list_recherche_age_max		= [];
		$list_recherche_nb_enfants	= [];
	...
	function step7($messages = []) {
		...
		// si l'user est log et arrive sur l'�tape 7 (ne prend donc pas le step7submit)
	...
	function step8($messages = []) {
		...
		if($user->id) {
			...
			// r�cup la recherche de l'utilisateur
		...
		// g�n�re le moteur de recherche avec les param�tres de recherche de l'utilisateur
		$list =& (new FCT())->getSearchEngine(JL::getSession('search_nb_enfants', 0), JL::getSession('search_recherche_age_min', 18), JL::getSession('search_recherche_age_max', 70), JL::getSession('search_canton_id', 0), JL::getSession('search_ville_id', 0), JL::getSession('search_username', ''), JL::getSession('search_online', 0), JL::getSession('search_titre', '')); // todo: compl�ter les champs
	...
	function step9submit() {
		...
		// t�l�phone
		if(strlen(JL::getSession('telephone', '', false)) == 0) {
	...
	function parrainagesubmit() {
		...
		if(!count($messages)) {
			...
			$rows 	= preg_split("/[^A-Za-z0-9.@_\-]/", (string) $row->emails);
			...
			foreach($rows as $email) {
				...
				if(preg_match('/^[A-Za-z0-9._-]+@[A-Za-z0-9.-]{2,}[.][A-Za-z]{2,3}$/', $email)) {
					...
					// check si l'email n'est pas d�j� enregistr�
					...
					if(!$emailExistant) {
						...
						// ajout de l'email dans le talbeau d'emails � ins�rer dans la DB
						$emails[]	= $email;
			...
			// s'il y a des emails � enregistrer
			if (is_array($emails)) {
	...
	/*function confirmation() {
		...
		if($user_id) {
			...
			// d�termine le champ � mettre � jour
			if($user_genre == 'h') {
				$field	= 'papa';
			} else {
				$field	= 'maman';
			...
			// d�truit la session par mesure de s�curit�
			JL::sessionDestroy();
	...
	function profil() {
		...
		if($profil->id) {
			...
			// le membre a-t-il accept� de montrer les photos de ses enfants ?
