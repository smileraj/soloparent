...
if($telelength==6 || $telephoneval=='+41-' || $telephoneval==''||$telephoneval=='+41') {
		...
		if(!count($messages)) {
			...
			if($user->id) {
				...
				// pseudo d�j� pris
				if($pseudoExistantUser || $pseudoExistantTmp || $pseudoSupprUser) {
...
$confrm_url="http://www.parentsolo.swiss/index.php?lang=".$_GET['lang']."&app=confrm&action=confirmation&urlid=".$confrm_encripted_url."&cnfrmid=".$confrm_encripted_id;
$site_url='http://www.parentsolo.swiss/';
...
$age_max_pro=(($values_select->userage)+5);
		...
		// r�cup le dernier appel � t�moins
			...
			if(is_dir($dir)) {
				...
				while($file = trim(readdir($dir_id))) {
					if(preg_match('#^temp.*'.$photo_type.'#', $file)) {
						copy($dir.'/'.$file, $dest_dir.'/'.str_replace('temp-', 'parent-', $file));
						@unlink($dir.'/'.$file);
